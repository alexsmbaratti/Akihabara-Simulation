using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.AI;

public class Pedestrians : MonoBehaviour {

	public GameObject goal;
	public float dis;
	public float initTime;

	// Use this for initialization
	void Start () {
		goal = GameObject.FindGameObjectsWithTag("PedestrianDestination")[Random.Range(0, GameObject.FindGameObjectsWithTag("PedestrianDestination").Length)];
		NavMeshAgent agent = GetComponent<NavMeshAgent>();
		GetComponent<NavMeshAgent>().speed = 15.0f;
		agent.SetDestination(new Vector3(goal.GetComponent<Transform>().position.x, goal.GetComponent<Transform>().position.y, goal.GetComponent<Transform>().position.z));
		initTime = Time.time;
	}
	
	// Update is called once per frame
	void Update () {
		if (GetComponent<NavMeshAgent>().isStopped == false) {
			dis = Vector3.Distance(transform.position, goal.GetComponent<Transform>().position);
			if (dis < 2) {
				int temp = PlayerPrefs.GetInt("AgentsProcessed");
				temp++;
				PlayerPrefs.SetInt("AgentsProcessed", temp);
				float temp1 = PlayerPrefs.GetFloat("AverageTime");
				temp1 += (Time.time - initTime);
				PlayerPrefs.SetFloat("AverageTime", temp1);
				Destroy(gameObject);
			}
		}
	}
}
